...
namespace Your_Ride.Repository.TimeRepo
{
    public class TimeRepository : Repository<Time> , ITimeRepository
    {
        ...
        {
            List<Time> times = await context.Times.Include(x=>x.Bus).Include(x=>x.Appointment).ToListAsync();
            return times;
        }

        public async Task<Time> GetTimeByID(int id)
        {
            Time time = await context.Times.Include(x => x.Bus).Include(x => x.Appointment).FirstOrDefaultAsync(x=>x.Id==id);
            return time;
        }
        public async Task<List<Time>> GetAllTimesByBusID(int id)
        {
            List<Time> times = await context.Times.Include(x=>x.Bus).Include(x=>x.Appointment).Where(x=>x.BusID==id).ToListAsync();
            return times;
        }
        public async Task<List<Time>> GetAllTimesByAppointmentID(int id)
        {
            List<Time> times = await context.Times.Include(x => x.Bus).Include(x => x.Appointment).Where(x => x.AppointmentId == id).ToListAsync();
            return times;
        }
        ...
        public async Task<Time> CreateTime(Time time)
        {
            ...
            Appointment existingAppointment = await context.Appointments.Include(x=>x.Times).FirstOrDefaultAsync(x => x.Id == time.Id);
